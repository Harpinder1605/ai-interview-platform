...
const API_BASE_URL = import.meta.env.VITE_API_URL || (isLocal ? 'http://localhost:8000' : 'https://ai-interview-platform-sjvq.onrender.com');
...
export default function Register() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // If successful, instantly redirect to the login page
      navigate('/login');
    } catch (err: any) {
  ...
  return (
    <div className="min-h-screen text-slate-100 flex items-center justify-center p-4 relative overflow-hidden font-sans">
      ...
      <div className="bg-linear-to-b from-slate-800/80 to-slate-900/90 p-8 rounded-3xl shadow-[inset_0_1px_1px_rgba(255,255,255,0.15),0_20px_40px_rgba(0,0,0,0.5)] max-w-md w-full border border-slate-500/30 backdrop-blur-xl z-10 animate-[fadeIn_0.5s_ease-out]">
        ...
        <h2 className="text-3xl font-extrabold text-white mb-2 text-center tracking-tight">Create an Account</h2>
        <p className="text-slate-400 text-center mb-8 text-sm">Join to elevate your professional interview skills.</p>
        ...
        <p className="mt-8 text-center text-sm text-slate-400">
          Already have an account? <Link to="/login" className="text-blue-400 hover:text-blue-300 font-semibold transition-colors">Log in</Link>
        </p>
      </div>
      <style>{`
        @keyframes fadeIn {
